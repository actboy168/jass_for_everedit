...
def build_jass_mac(cj, bj):
    ...
    syntax.block('type')
    jass.AddItem(jass.CreateWord(COLOR_WORD2, 'hashtable integer real boolean string handle agent code event player widget unit destructable item ability buff force group trigger triggercondition triggeraction timer location region rect boolexpr sound conditionfunc filterfunc unitpool itempool race alliancetype racepreference gamestate igamestate fgamestate playerstate playerscore playergameresult unitstate aidifficulty eventid gameevent playerevent playerunitevent unitevent limitop widgetevent dialogevent unittype gamespeed gamedifficulty gametype mapflag mapvisibility mapsetting mapdensity mapcontrol playerslotstate volumegroup camerafield camerasetup playercolor placement startlocprio raritycontrol blendmode texmapflags effect effecttype weathereffect terraindeformation fogstate fogmodifier dialog button quest questitem defeatcondition timerdialog leaderboard multiboard multiboarditem trackable gamecache version itemtype texttag attacktype damagetype weapontype soundtype lightning pathingtype image ubersplat nothing', True))
    ...
    syntax.block('extra')
    syntax.Call(jass.FoldText(r'^\s*\b(globals|library|struct|scope|method|interface|function|if|loop|private\s+function|public\s+function|static\s+function|private\s+method|public\s+method|static\s+method|private\s+struct|public\s+struct|static\s+struct)\b', True))
